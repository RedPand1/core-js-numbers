...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const chislitel = x1 * x2 + y1 * y2;
  const znamenatel1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const znamenatel2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const znamenatel = znamenatel1 * znamenatel2;
  return Math.acos(chislitel / znamenatel);
}
...
function roundToPowerOfTen(num, pow) {
  const dec = 10 ** pow;
  const set = num % dec;
  return num - set + Math.round(set / dec) * dec;
}
